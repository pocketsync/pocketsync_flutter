  ...
  group('SchemaManager', () {
    ...
    group('initializePocketSyncTables', () {
      test('should create all required PocketSync tables', () async {
        ...
        expect(indexes.length, greaterThanOrEqualTo(5)); // At least 5 indexes should be created
      });
    });
    ...
    group('getUserTables', () {
      test('should return only user tables', () async {
        ...
        final allTableNames = allTables.map((t) => t['name'] as String).toList();
        ...
        final usersHasGlobalId = usersColumns.any((col) => 
            col['name'] == SyncConfig.defaultGlobalIdColumnName);
        final productsHasGlobalId = productsColumns.any((col) => 
            col['name'] == SyncConfig.defaultGlobalIdColumnName);
    ...
    group('setupChangeTracking', () {
      test('should add global ID column and create triggers for all user tables', () async {
        // Arrange
        await schemaManager.initializePocketSyncTables(db);
        ...
        final triggers = await db.query(
          'sqlite_master',
          where: "type = 'trigger' AND (name LIKE 'after_insert_%' OR name LIKE 'after_update_%' OR name LIKE 'after_delete_%')",
        );
      ...
      test('should generate global IDs for existing records', () async {
        ...
        await db.insert('users', {'name': 'Test User 1', 'email': 'user1@example.com'});
        await db.insert('users', {'name': 'Test User 2', 'email': 'user2@example.com'});
    ...
    group('reset', () {
      test('should reset all PocketSync state', () async {
        ...
        await db.insert('users', {'name': 'Test User', 'email': 'user@example.com'});
    ...
    group('cleanupOldSyncRecords', () {
      test('should delete old synced records based on retention policy', () async {
        // Arrange
        await schemaManager.initializePocketSyncTables(db);
        ...
        final deletedCount = await schemaManager.cleanupOldSyncRecords(db, options);
        ...
        expect((deviceState.first['last_cleanup_timestamp'] as int), greaterThan(oldTimestamp));
      });
    });
    ...
    group('generateUpdateCondition', () {
      test('should generate correct SQL condition for detecting changes', () {
        // Arrange
        final columns = ['id', 'name', 'email', SyncConfig.defaultGlobalIdColumnName];
        ...
        expect(condition, isNot(contains('OLD.${SyncConfig.defaultGlobalIdColumnName}')));
      });
    });
    ...
    group('syncPreExistingData', () {
      test('should create change records for existing data', () async {
        ...
        // Insert test data
        await db.insert('users', {'name': 'Existing User 1', 'email': 'existing1@example.com'});
        await db.insert('users', {'name': 'Existing User 2', 'email': 'existing2@example.com'});
      ...
      test('should not process tables that were already processed', () async {
        ...
        // Insert test data
        await db.insert('users', {'name': 'Existing User', 'email': 'existing@example.com'});
