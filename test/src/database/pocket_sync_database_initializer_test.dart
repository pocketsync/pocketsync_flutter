...
void main() {
  ...
  group('PocketSyncDatabaseInitializer', () {
    test('initializePocketSyncTables creates all required system tables', () async {
      ...
      final changesTableInfo = await db.rawQuery('PRAGMA table_info(__pocketsync_changes)');
      expect(changesTableInfo.length, 8); // Should have 8 columns
      
      // Verify column names
      final changesColumns = changesTableInfo.map((c) => c['name'] as String).toList();
      expect(changesColumns, containsAll([
        'id', 'table_name', 'record_rowid', 'operation', 
        'timestamp', 'data', 'version', 'synced'
      ]));
      ...
      final indexNames = changesIndexes.map((i) => i['name'] as String).toList();
    ...
    test('getUserTables returns only user tables', () async {
      ...
      await db.execute('CREATE TABLE __pocketsync_test (id INTEGER PRIMARY KEY)');
      await db.execute('CREATE TABLE test_sqlite_table (id INTEGER PRIMARY KEY)');
    ...
    test('setupChangeTracking adds ps_global_id column and creates triggers', () async {
      // Create a test table
      await db.execute('CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)');
    ...
    test('backupTriggers saves trigger definitions', () async {
      ...
      final triggerNames = backups.map((b) => b['trigger_name'] as String).toList();
      expect(triggerNames, containsAll([
        'after_insert_users',
        'after_update_users',
        'after_delete_users',
    ...
    test('initializeTableVersions creates version records for all tables', () async {
      // Create test tables
      await db.execute('CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)');
      await db.execute('CREATE TABLE products (id INTEGER PRIMARY KEY, title TEXT)');
    ...
    test('syncPreExistingRecords creates change records for existing data', () async {
      // Create a test table
      await db.execute('CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)');
      ...
      for (final change in changes) {
        ...
        expect(dataString.contains('John') || dataString.contains('Jane'), isTrue);
      }
