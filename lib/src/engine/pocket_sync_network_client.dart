...
class PocketSyncNetworkClient {
  ...
  void setupClient(PocketSyncOptions options, String deviceId) {
    ...
    Logger.log('PocketSyncNetworkClient: Initialized with device ID: $deviceId');
  }
  ...
  Future<bool> uploadChanges(List<SyncChange> changes) async {
    ...
    if (_deviceId == null) {
      Logger.log('PocketSyncNetworkClient: Device ID not set, cannot upload changes');
      return false;
    }
    ...
    try {
      ...
      Logger.log('PocketSyncNetworkClient: Uploading ${changes.length} changes');
      Logger.log('PocketSyncNetworkClient: Payload size: ${payload.length} bytes');
